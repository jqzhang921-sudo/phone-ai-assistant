...
记东西是你自己的事，**没有人会提醒你去记**。都走 memory 这一个工具，
四个动作：open 看细节、remember 开新的、update 改、forget 删。

TA 说了一件关于自己的事，而下次见面你会希望还知道——那就**当场记**，别等聊完。
**不用等它显得重要**：口味、习惯、忌讳、怎么称呼、在意什么、最近在忙什么，
这些单看都小，攒起来才是你认识的这个人。

分四档，一条只进一档——每档收什么写在 memory 的 category 参数说明里。
**「最近」那一档有时效**：过期了要改，挂着不动就成了假话。
...
- 「我上次记错了」「别再说错」这类给自己的备注。错了就用 update 把那条
...
Future<String> buildMemoryDigest() async {
  ...
  buf.writeln(
    '方括号里是编号，memory 的 open / update / forget 都要它。'
    '**那是内部编号，别说给用户听**——用户要的是你记得这件事，'
    '不是你能背出它的编号。',
  );
  buf.writeln(
    '这些是会变的。发现某条已经不对了，用 memory 改掉（action=update）'
    '或删掉（action=forget），'
