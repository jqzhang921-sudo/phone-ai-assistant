...
class ToolTiers {
  ...
  static const resident = <String>{
    ...
    'write_diary_entry',
    'recall_records',
    // TA 随手发张图，它当场就得能决定要不要拿去当头像。放延迟层的话
    // 它根本想不到去搜「换头像」，这个功能就等于没有。
    'set_avatar',
    // 表情是「说话的一部分」，聊到一半才想得起来发。放延迟层的话它根本想不到
    // 去搜「表情」，这个功能就等于没有——和 glance_screen 同一个理由。
    'send_sticker',
    // 「有几条路，你想走哪条」是聊到一半才冒出来的念头。放延迟层它想不到去搜，
    // 于是永远不会问，只会自己替她定——那正是这个工具要避免的。
    'ask_choice',
    // TA 随口一句「你看看」，它当场就得能看。放延迟层的话它想不到去搜，
    // 只会回一句「我看不到你的屏幕」。
    'glance_screen',
...
class ToolFinder {
  ...
  static Future<Map<String, dynamic>> execute(Map<String, dynamic> args) async {
    ...
    final scored =
        pool.map((t) => (t, _score(query, t))).toList()
