  ...
  static Future<Map<String, dynamic>> execute(Map<String, dynamic> args) async {
    try {
      ...
      if (key == null || key.isEmpty) {
        return {'success': false, 'error': '用户还没有在设置里填微信读书 API Key，查不了划线。'};
      }
    ...
    try {
      ...
      if (rawTitle.isEmpty) {
        return {
          'success': false,
          'error':
              'title 参数为空。请重新调用 get_book_highlights 并给出书名，'
              '例如 {"title": "白夜行"}。',
        };
      }
      ...
      return {
        ...
        if (book.author != null && book.author!.isNotEmpty)
          'author': book.author,
        'highlights':
            truncated ? '${text.substring(0, _kMaxHighlightChars)}…' : text,
        if (truncated)
          'note':
              '划线太多，只回了前 $_kMaxHighlightChars 字，'
