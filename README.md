...
<p align="center">
  <img src="docs/home.png" width="45%" alt="首页">
  &nbsp;&nbsp;
  <img src="docs/chat.png" width="45%" alt="聊天">
</p>
...
上面那张聊天截图就是这套约束的样子：一句话说完、没有反问、没有 emoji、没有「你是不是心情不好」这类对状态的复述。
...
```mermaid
flowchart TB
    subgraph prefix["system 前缀 · 跨轮次逐字节稳定 · 吃 KV 缓存"]
        direction LR
        A["名字<br/>几乎不变"] --> B["人设<br/>const"] --> C["读记录的规则<br/>const"] --> D["记忆摘要<br/>记忆改了才变"]
    end

    subgraph history["对话历史"]
        E["早期消息折成摘要 + 最近 45 条原文"]
    end

    subgraph tail["最后一条用户消息尾部 · 每轮全额重付"]
        F["今天的日记 · 一隅收藏 · 在读的书 · 有没有信"]
    end

    prefix --> history --> tail
```

越靠前的越不该变。缓存是逐字节从头比对的，把易变的放前面，后面几千 token 的对话历史会整段作废。

所以「TA 是谁」（记忆摘要）在前缀里常驻，「最近发生了什么」挂在尾部——后者天天变，本来就吃不到缓存，放前面只会连累别人。
